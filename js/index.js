...
const menu_data = {
	...
	url: [
		...
		},
		{
			...
			content: '削除',
			command: (id, node) => {
				confirm('リンクを削除', 'この操作は取り消せません。', '削除', () => {
					bookmark.remove(id, () => {
						render_node(node)
					})
				})
	...
	folder: [
		...
		{ type: 'partition' },
		{
			...
			children: [
				...
				{
					type: 'command',
					content: 'フォルダ内をすべて展開して削除',
					command: (id, node) => {
						bookmark.get(id, ([folder_data]) => {
							bookmark.deployFolder(folder_data.parentId, id, folder_data.index, true)
							close_id(id)
							bookmark.remove(id, () => {
								render_node(node)
							})
							...
							bookmark.create(
								...
								current_folder => {
									...
									while (children.length !== 0) {
										...
										bookmark.create(
											{ parentId: current_folder.id, title: `分割フォルダ${count}` },
											division_folder => {
		...
		},
		{
			...
			content: '削除',
			command: (id, node) => {
				confirm('フォルダを削除', 'この操作は取り消せません。', '削除', () => {
					bookmark.remove(id, () => {
						render_node(node)
						close_id(id)
					})
...
function open_path(path) {
	...
	open_folder(path, 1, () => {
		...
		dialog('初期エラー', ['デフォルトのパスが見つかりません。', path.join('>')])
	})
}
...
function add_wrapper(id, node, collapse) {
	...
	collapse_button.addEventListener('click', function () {
		if (wrapper.classList.contains('collapse')) wrapper.classList.remove('collapse')
		else wrapper.classList.add('collapse')
	})
	// close_button
	if (node !== 0) {
	...
	content.addEventListener('dragstart', function (e) {
		if (e.target.classList && e.target.classList.contains('entry-move')) {
			...
			dragParent = {
				node,
				open: open_list[node + 1] && dragEl.dataset.id === open_list[node + 1].dataset.id,
			}
...
function close(index) {
	open_list.slice(index).forEach(folder => {
		folder.remove()
	})
...
function add_menu(e, type, target, id, node) {
	...
	menu.style.left =
		e.pageX - (window.innerWidth < e.pageX + menu.offsetWidth ? menu.offsetWidth : 0) + 'px'
	menu.style.top =
		e.pageY - (window.innerHeight < e.pageY + menu.offsetHeight ? menu.offsetHeight : 0) + 'px'
}
...
function remove_menu() {
	if (menu) {
		...
		if (menu_target) {
			menu_target.classList.remove('menu-target')
		}
		menu.remove()
		menu = null
	}
...
function close_id(id) {
	...
	let wrapper_data = open_list.map(wrapper => ({
		node: Number(wrapper.dataset.node),
		id: wrapper.dataset.id,
	}))
	wrapper_data = wrapper_data.filter(data => id.includes(data.id))
	wrapper_data.forEach(({ node }) => {
		close(node)
	})
}

function time_to_string(epoch_ms, default_value) {
	if (epoch_ms === undefined) {
